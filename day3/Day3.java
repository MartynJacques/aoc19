...
public class Day3 {
  ...
  private static enum Direction {
    ...
    public static Direction fromString(String directionString) {
      ...
      System.err.println("somethings gone wrong in fromString enum Direction");
      return DOWN;
    }
  }

  public static ArrayList<Coordinate> getCoordinatesFromDirection(
      ArrayList<Coordinate> currentRoute, String directionWithMagnitude) {
    ArrayList<Coordinate> newRoute = new ArrayList<>();
    Direction direction = Direction.fromString(directionWithMagnitude.substring(0, 1));
    int magnitude = Integer.parseInt(directionWithMagnitude.substring(1));
    Coordinate lastCoordinate = currentRoute.get(currentRoute.size() - 1);
    for (int i = 1; i <= magnitude; i++) {
      switch (direction) {
        case RIGHT:
          newRoute.add(new Coordinate(lastCoordinate.xCord + i, lastCoordinate.yCord));
          break;
        case LEFT:
          newRoute.add(new Coordinate(lastCoordinate.xCord - i, lastCoordinate.yCord));
          break;
        case UP:
          newRoute.add(new Coordinate(lastCoordinate.xCord, lastCoordinate.yCord + i));
          break;
        case DOWN:
          newRoute.add(new Coordinate(lastCoordinate.xCord, lastCoordinate.yCord - i));
          break;
  ...
  public static ArrayList<Coordinate> getRoute(String input) {
    String[] movements = input.split(",");
    ArrayList<Coordinate> currentRoute = new ArrayList<>();
    currentRoute.add(new Coordinate(0, 0)); // Start from the origin
    for (String movement : movements) {
      currentRoute.addAll(getCoordinatesFromDirection(currentRoute, movement));
    }
    return currentRoute;
  }
  ...
  public static void main(String[] args) {
    ...
    ArrayList<Coordinate> routeA = getRoute(inputA);
    ArrayList<Coordinate> routeB = getRoute(inputB);

    Set<Coordinate> intersection =
        routeA.stream().distinct().filter(routeB::contains).collect(Collectors.toSet());
    ...
    for (Coordinate c : intersection) {
      int manDist = manhattanDistance(origin, c);
      if (manDist < min) {
        min = manDist;
      }
    }
    System.out.println("Min manhattan distance:");
    System.out.println(min);

    // Part two
    int minDist = Integer.MAX_VALUE;
    for (Coordinate c : intersection) {
      int sum = routeA.indexOf(c) + routeB.indexOf(c);
      if (sum < minDist) {
        minDist = sum;
      }
    }
    System.out.println("Min walking dist:");
